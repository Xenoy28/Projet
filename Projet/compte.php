...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif (!empty($age) && (!is_numeric($age) || $age < 15 || $age > 100)) {
        $erreur = 'Âge invalide (entre 15 et 100).';
    } else {
        // Vérifier si l'email est pris par un autre utilisateur
        $check = $pdo->prepare('SELECT id_utilisateur FROM utilisateur WHERE email = ? AND id_utilisateur != ?');
        $check->execute([$email, $user['id']]);
        if ($check->fetch()) {
            $erreur = 'Cette adresse e-mail est déjà utilisée par un autre compte.';
        } else {
            $pdo->prepare('UPDATE utilisateur SET nom = ?, prenom = ?, age = ?, email = ?, numero = ? WHERE id_utilisateur = ?')
                    ->execute([$nom, $prenom, $age ?: null, $email, $numero ?: null, $user['id']]);
...
<head>
    ...
    <title>SERENITY – Mon Profil</title>
...
<div style="width:100%;max-width:480px;">
    ...
    <div class="login-card">
        ...
        <div class="login-body">
            ...
            <!-- Avatar / info actuelle -->
            <div style="background:var(--blue-pale);border:1px solid var(--border);border-radius:10px;padding:12px 16px;margin-bottom:20px;display:flex;align-items:center;gap:12px;">
                <div style="width:44px;height:44px;border-radius:50%;background:var(--navy-mid);border:2px solid var(--blue);display:flex;align-items:center;justify-content:center;font-size:1.2rem;flex-shrink:0;">
                    👤
                </div>
                <div>
                    <div style="font-size:.78rem;color:#64748B;">Compte connecté</div>
                    <div style="font-size:.95rem;color:#E2E8F0;font-weight:700;"><?= htmlspecialchars($profil['prenom'].' '.$profil['nom']) ?></div>
                    <div style="font-size:.78rem;color:#475569;"><?= htmlspecialchars($profil['email']) ?></div>
                </div>
            </div>
            ...
            <form method="POST" novalidate>
                ...
                <div class="row g-3 mb-3">
                    <div class="col-6">
                        <label class="form-label">Nom <span style="color:#F87171">*</span></label>
                        <div class="input-group">
                            <span class="input-group-text"><i class="bi bi-person"></i></span>
                            <input type="text" name="nom" class="form-control" placeholder="Dupont"
                                   value="<?= htmlspecialchars($_POST['nom'] ?? $profil['nom']) ?>" required>
                        </div>
                    </div>
                    <div class="col-6">
                        <label class="form-label">Prénom <span style="color:#F87171">*</span></label>
                        <div class="input-group">
                            <span class="input-group-text"><i class="bi bi-person"></i></span>
                            <input type="text" name="prenom" class="form-control" placeholder="Marie"
                                   value="<?= htmlspecialchars($_POST['prenom'] ?? $profil['prenom']) ?>" required>
                        </div>
                    </div>
                </div>
                ...
                <!-- Email -->
                <div class="mb-3">
                    <label class="form-label">Adresse e-mail <span style="color:#F87171">*</span></label>
                    <div class="input-group">
                        <span class="input-group-text"><i class="bi bi-envelope"></i></span>
                        <input type="email" name="email" class="form-control" placeholder="exemple@email.com"
                               value="<?= htmlspecialchars($_POST['email'] ?? $profil['email']) ?>" required>
                    </div>
                </div>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-5">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" name="age" class="form-control" placeholder="20" min="15" max="100"
                                   value="<?= htmlspecialchars($_POST['age'] ?? $profil['age']) ?>">
                        </div>
                    </div>
                    <div class="col-7">
                ...
                <button type="submit" class="btn-serenity">
                    <i class="bi bi-person-check me-2"></i>Enregistrer les modifications
                </button>
            </form>
            ...
            <div class="text-center">
                <a href="mdp.php" style="color:var(--blue);font-weight:600;text-decoration:none;">
                    <i class="bi bi-shield-lock me-1"></i>Modifier mon mot de passe →
                </a>
            </div>
    ...
    <div class="text-center">
        <a href="index.php" class="back-link"><i class="bi bi-house"></i> Accueil SERENITY</a>
    </div>
</div>
