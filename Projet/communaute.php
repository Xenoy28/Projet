<?php
session_start();

$user = $_SESSION['utilisateur'] ?? null;

$logoB64 = '';

$categories = [
    ['slug'=>'stress',     'label'=>'Stress / Anxiété',  'emoji'=>'😟', 'class'=>'tag-red'],
    ['slug'=>'isolement',  'label'=>'Isolement',         'emoji'=>'🌙', 'class'=>'tag-blue'],
    ['slug'=>'handicap',   'label'=>'Handicap invisible','emoji'=>'👁️', 'class'=>'tag-purple'],
    ['slug'=>'precarite',  'label'=>'Précarité',         'emoji'=>'💰', 'class'=>'tag-red'],
    ['slug'=>'orientation','label'=>'Orientation',       'emoji'=>'🎓', 'class'=>'tag-blue'],
    ['slug'=>'autre',      'label'=>'Autre',             'emoji'=>'💬', 'class'=>'tag-purple'],
];

$discussions = [
    [
        'auteur'=>'Marie', 'initiale'=>'M',
        'titre'=>'Comment gérer le stress avant les partiels ?',
        'message'=>"Je me sens dépassée avec les examens qui arrivent. Vous avez des techniques qui marchent vraiment ?",
        'categorie'=>'stress', 'reponses'=>5, 'likes'=>8, 'date'=>'Il y a 2h',
    ],
    [
        'auteur'=>'Lucas', 'initiale'=>'L',
        'titre'=>'Cherche groupe de révision en maths',
        'message'=>"Quelqu'un pour réviser les maths ensemble cette semaine ? Je suis en L1 à Paris.",
        'categorie'=>'orientation', 'reponses'=>2, 'likes'=>3, 'date'=>'Il y a 5h',
    ],
    [
        'auteur'=>'Sofia', 'initiale'=>'S',
        'titre'=>"Ressources pour le handicap invisible à l'université",
        'message'=>"Je viens d'apprendre que j'ai un TDAH. Je ne sais pas par où commencer pour obtenir des aménagements.",
        'categorie'=>'handicap', 'reponses'=>7, 'likes'=>14, 'date'=>'Hier',
    ],
    [
        'auteur'=>'Inès', 'initiale'=>'I',
        'titre'=>'Aides financières d\'urgence pour étudiants ?',
        'message'=>"Mon contrat a été rompu et je ne sais pas comment finir le mois. Quelles aides existent ?",
        'categorie'=>'precarite', 'reponses'=>4, 'likes'=>11, 'date'=>'Il y a 3j',
    ],
];

$filtre = $_GET['categorie'] ?? '';

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['titre'])) {
    array_unshift($discussions, [
        'auteur'    => $user['prenom'] ?? 'Anonyme',
        'initiale'  => strtoupper(substr($user['prenom'] ?? 'A', 0, 1)),
        'titre'     => htmlspecialchars($_POST['titre']),
        'message'   => htmlspecialchars($_POST['message']),
        'categorie' => htmlspecialchars($_POST['categorie'] ?? 'autre'),
        'reponses'  => 0, 'likes' => 0, 'date' => "À l'instant",
    ]);
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['supprimer_index'])) {
    $idx = (int) $_POST['supprimer_index'];
    $auteurDisc = $discussions[$idx]['auteur'] ?? '';
    $prenomUser = $user['prenom'] ?? '';
    if ($prenomUser && $auteurDisc === $prenomUser) {
        array_splice($discussions, $idx, 1);
    }
}

$liste = $filtre
    ? array_values(array_filter($discussions, fn($d) => $d['categorie'] === $filtre))
    : $discussions;

function getCatProp(string $slug, array $cats, string $prop): string {
    foreach ($cats as $c) { if ($c['slug'] === $slug) return $c[$prop]; }
    return '';
}
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>SERENITY – Communauté</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css" rel="stylesheet">
    <link href="style.css" rel="stylesheet">
    <style>
        .disc-card {
            background: var(--navy-card);
            border: 1px solid var(--border);
            border-radius: 16px;
            padding: 22px 26px;
            margin-bottom: 14px;
            transition: border-color .2s, box-shadow .2s;
        }
        .disc-card:hover {
            border-color: rgba(0,135,255,.35);
            box-shadow: 0 4px 20px rgba(0,135,255,.07);
        }
        .disc-avatar {
            width: 42px; height: 42px; border-radius: 50%;
            background: var(--blue);
            display: flex; align-items: center; justify-content: center;
            font-weight: 700; font-size: .95rem; color: #fff; flex-shrink: 0;
        }
        .disc-title { font-size:1rem; font-weight:700; color:#fff; margin-bottom:6px; }
        .disc-msg   { color:var(--text-light); font-size:.88rem; line-height:1.6; margin-bottom:12px; }
        .disc-meta  { font-size:.8rem; color:var(--text-muted); display:flex; align-items:center; gap:14px; flex-wrap:wrap; }
        .like-btn {
            background: none; border: 1px solid var(--border); border-radius: 20px;
            color: var(--text-muted); font-size:.8rem; padding:3px 10px;
            cursor:pointer; transition:all .2s; display:flex; align-items:center; gap:4px;
        }
        .like-btn:hover, .like-btn.liked { border-color:#F87171; color:#F87171; background:rgba(248,113,113,.08); }
        .filter-pill {
            padding: 6px 14px; border-radius: 20px; border: 1px solid var(--border);
            background: rgba(0,135,255,.05); color: var(--text-muted); font-size:.8rem;
            cursor:pointer; text-decoration:none; transition:all .2s; display:inline-block;
        }
        .filter-pill:hover, .filter-pill.active { border-color:var(--blue); color:#fff; background:rgba(0,135,255,.15); }
        .sidebar-box {
            background: var(--navy-card); border: 1px solid var(--border);
            border-radius: 16px; padding: 20px; margin-bottom: 20px;
        }
        .sidebar-box h6 {
            font-size:.75rem; font-weight:700; color:var(--text-muted);
            text-transform:uppercase; letter-spacing:.5px; margin-bottom:14px;
        }
        .grp-row {
            display:flex; align-items:center; justify-content:space-between;
            padding:8px 0; border-bottom:1px solid rgba(0,135,255,.08);
            color:var(--text-light); font-size:.88rem;
        }
        .grp-row:last-child { border-bottom:none; }
        .badge-count {
            background:rgba(0,135,255,.12); color:var(--blue);
            border-radius:10px; padding:2px 8px; font-size:.75rem; font-weight:600;
        }
        .form-control, .form-select {
            background: rgba(255,255,255,.04) !important;
            border: 1px solid var(--border) !important;
            color: #fff !important; border-radius: 10px !important;
        }
        .form-control:focus, .form-select:focus {
            box-shadow: 0 0 0 3px rgba(0,135,255,.15) !important;
            border-color: var(--blue) !important;
        }
        .form-control::placeholder { color:#64748B; }
        .form-select option { background:#0D1F3C; }
        .tag { display:inline-block; padding:3px 10px; border-radius:20px; font-size:.75rem; font-weight:600; margin-bottom:8px; }
    </style>
</head>
<body>

<!-- ═══════════ NAVBAR ═══════════ -->
<nav class="navbar navbar-expand-lg sticky-top">
    <div class="container">
        <a class="navbar-brand" href="index.php">
            <div class="logo-circle">
                <img src="data:image/jpeg;base64,<?= $logoB64 ?>" alt="Logo">
            </div>
            SERENITY
        </a>
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navMain">
            <i class="bi bi-list" style="color:var(--blue);font-size:1.3rem;"></i>
        </button>
        <div class="collapse navbar-collapse" id="navMain">
            <ul class="navbar-nav mx-auto gap-1">
                <li class="nav-item"><a class="nav-link" href="index.php">Accueil</a></li>
                <li class="nav-item"><a class="nav-link" href="temoignages.php">Témoignages</a></li>
                <li class="nav-item"><a class="nav-link" href="ressources.php">Ressources</a></li>
                <li class="nav-item"><a class="nav-link" href="mon-espace.php">Mon Espace</a></li>
                <li class="nav-item"><a class="nav-link active" href="communaute.php">Communauté</a></li>
                <li class="nav-item">
                    <a class="nav-link urgence" href="urgences.php">
                        <i class="bi bi-telephone-fill me-1"></i>Urgences
                    </a>
                </li>
            </ul>
            <div class="d-flex align-items-center gap-2">
                <input type="text" class="search-input d-none d-lg-block" placeholder="🔍 Rechercher…">
                <?php if ($user): ?>
                    <div class="dropdown">
                        <button class="btn-user dropdown-toggle" data-bs-toggle="dropdown">
                            <i class="bi bi-person-circle me-1"></i>
                            <?= htmlspecialchars($user['prenom']) ?> <?= htmlspecialchars($user['nom'][0]) ?>.
                        </button>
                        <ul class="dropdown-menu dropdown-menu-end">
                            <li>
                                <div class="px-3 py-2" style="border-bottom:1px solid var(--border);margin-bottom:4px;">
                                    <div style="font-size:.8rem;color:#64748B;">Connecté en tant que</div>
                                    <div style="font-size:.9rem;color:#fff;font-weight:600;"><?= htmlspecialchars($user['prenom'].' '.$user['nom']) ?></div>
                                    <div style="font-size:.78rem;color:#475569;"><?= htmlspecialchars($user['email']) ?></div>
                                </div>
                            </li>
                            <li><a class="dropdown-item" href="mon-espace.php"><i class="bi bi-person me-2" style="color:var(--blue)"></i>Mon Espace</a></li>
                            <li><a class="dropdown-item" href="communaute.php"><i class="bi bi-people me-2" style="color:var(--blue)"></i>Communauté</a></li>
                            <li><hr class="dropdown-divider"></li>
                            <li><a class="dropdown-item text-danger" href="logout.php"><i class="bi bi-box-arrow-right me-2"></i>Déconnexion</a></li>
                        </ul>
                    </div>
                <?php else: ?>
                    <a href="login.php" class="btn-nav-login">
                        <i class="bi bi-box-arrow-in-right me-1"></i>Connexion
                    </a>
                <?php endif; ?>
            </div>
        </div>
    </div>
</nav>

<!-- ═══════════ HERO ═══════════ -->
<section class="hero">
    <div class="container">
        <h1>Espace <span>Communauté</span> 💬</h1>
        <p>Discute avec d'autres étudiants, partage ton expérience et trouve du soutien. Tu n'es pas seul(e).</p>
        <div class="d-flex gap-3 justify-content-center flex-wrap">
            <a href="#nouvelle-discussion" class="btn-hero-primary"><i class="bi bi-pencil-fill me-2"></i>Créer une discussion</a>
            <a href="ressources.php" class="btn-hero-outline"><i class="bi bi-book me-2"></i>Voir les ressources</a>
        </div>
    </div>
</section>

<div class="blue-line"></div>

<!-- ═══════════ CONTENU ═══════════ -->
<div class="container mb-5">
    <div class="row g-4">

        <!-- SIDEBAR -->
        <div class="col-lg-3">

            <div class="sidebar-box">
                <h6>Filtrer par thème</h6>
                <div class="d-flex flex-column gap-2">
                    <a href="communaute.php" class="filter-pill <?php echo !$filtre ? 'active' : ''; ?>">
                        🔍 Toutes les discussions
                    </a>
                    <?php foreach ($categories as $cat): ?>
                        <a href="communaute.php?categorie=<?= $cat['slug'] ?>"
                           class="filter-pill <?php echo $filtre === $cat['slug'] ? 'active' : ''; ?>">
                            <?= $cat['emoji'] ?> <?= $cat['label'] ?>
                        </a>
                    <?php endforeach; ?>
                </div>
            </div>

            <div class="sidebar-box">
                <h6>Groupes populaires</h6>
                <div class="grp-row"><span>😟 Anxiété & Stress</span><span class="badge-count">120</span></div>
                <div class="grp-row"><span>🎓 Étudiants L1</span><span class="badge-count">85</span></div>
                <div class="grp-row"><span>👁️ Handicap invisible</span><span class="badge-count">50</span></div>
                <div class="grp-row"><span>💰 Précarité</span><span class="badge-count">38</span></div>
            </div>

            <div class="sidebar-box">
                <h6>La communauté</h6>
                <div class="grp-row"><span>👤 Membres</span><span class="badge-count">1 240</span></div>
                <div class="grp-row"><span>💬 Discussions</span><span class="badge-count">348</span></div>
                <div class="grp-row"><span>✅ En ligne</span>
                    <span class="badge-count" style="background:rgba(52,211,153,.15);color:#34D399;">23</span>
                </div>
            </div>

        </div>

        <!-- DISCUSSIONS + FORMULAIRE -->
        <div class="col-lg-9">

            <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
                <h4 style="color:#fff;font-weight:700;margin:0;">
                    <?php echo $filtre ? '🔍 Thème : ' . htmlspecialchars($filtre) : '💬 Dernières discussions'; ?>
                </h4>
                <span style="font-size:.82rem;color:#64748B;"><?= count($liste) ?> discussion(s)</span>
            </div>

            <?php if (empty($liste)): ?>
                <div class="disc-card text-center py-4">
                    <div style="font-size:2.5rem;margin-bottom:12px;">🔍</div>
                    <p style="color:#64748B;">Aucune discussion dans cette catégorie.</p>
                    <a href="communaute.php" class="btn-hero-outline" style="font-size:.85rem;">Voir toutes</a>
                </div>
            <?php else: ?>
                <?php foreach ($discussions as $idx => $d):
                    if ($filtre && $d['categorie'] !== $filtre) continue;
                    $estAuteur = $user && ($user['prenom'] === $d['auteur']);
                    ?>
                    <div class="disc-card" id="disc-<?= $idx ?>">
                        <div class="d-flex gap-3">
                            <div class="disc-avatar"><?= htmlspecialchars($d['initiale']) ?></div>
                            <div style="flex:1;min-width:0;">
                                <div class="d-flex align-items-start justify-content-between gap-2 flex-wrap mb-1">
                                <span class="tag <?= getCatProp($d['categorie'], $categories, 'class') ?>">
                                    <?= getCatProp($d['categorie'], $categories, 'emoji') ?>
                                    <?= getCatProp($d['categorie'], $categories, 'label') ?>
                                </span>
                                    <div class="d-flex align-items-center gap-2">
                                        <span style="font-size:.78rem;color:#64748B;white-space:nowrap;"><?= $d['date'] ?></span>
                                        <?php if ($estAuteur): ?>
                                            <form method="POST" style="margin:0;" onsubmit="return confirm('Supprimer cette discussion ?');">
                                                <input type="hidden" name="supprimer_index" value="<?= $idx ?>">
                                                <button type="submit" title="Supprimer"
                                                        style="background:none;border:1px solid rgba(239,68,68,.3);border-radius:8px;color:#F87171;font-size:.75rem;padding:2px 8px;cursor:pointer;transition:all .2s;"
                                                        onmouseover="this.style.background='rgba(239,68,68,.1)'"
                                                        onmouseout="this.style.background='none'">
                                                    <i class="bi bi-trash3-fill"></i>
                                                </button>
                                            </form>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                <div class="disc-title"><?= htmlspecialchars($d['titre']) ?></div>
                                <div class="disc-msg"><?= htmlspecialchars($d['message']) ?></div>
                                <div class="disc-meta">
                                <span style="color:#94A3B8;font-weight:600;">
                                    <i class="bi bi-person-fill me-1" style="color:var(--blue);"></i><?= htmlspecialchars($d['auteur']) ?>
                                </span>
                                    <span><i class="bi bi-chat-dots" style="color:var(--blue);"></i> <?= $d['reponses'] ?> réponse(s)</span>
                                    <button class="like-btn" onclick="toggleLike(this)">
                                        <i class="bi bi-heart-fill"></i>
                                        <span class="like-count"><?= $d['likes'] ?></span>
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>

            <!-- Formulaire nouvelle discussion -->
            <div class="disc-card mt-4" id="nouvelle-discussion">
                <h5 style="color:#fff;font-weight:700;margin-bottom:20px;">
                    <i class="bi bi-pencil-fill me-2" style="color:var(--blue);"></i>Créer une discussion
                </h5>

                <?php if (!$user): ?>
                    <div style="background:rgba(0,135,255,.06);border:1px solid var(--border);border-radius:12px;padding:20px;text-align:center;">
                        <p style="color:#94A3B8;margin-bottom:12px;">Connecte-toi pour participer aux discussions.</p>
                        <a href="login.php" class="btn-hero-primary" style="font-size:.88rem;">
                            <i class="bi bi-box-arrow-in-right me-1"></i>Se connecter
                        </a>
                    </div>
                <?php else: ?>
                    <form method="POST" action="communaute.php#nouvelle-discussion">
                        <div class="mb-3">
                            <label style="font-size:.82rem;font-weight:600;color:var(--text-light);text-transform:uppercase;letter-spacing:.3px;display:block;margin-bottom:6px;">Thématique *</label>
                            <select name="categorie" class="form-select">
                                <?php foreach ($categories as $cat): ?>
                                    <option value="<?= $cat['slug'] ?>"><?= $cat['emoji'] ?> <?= $cat['label'] ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="mb-3">
                            <label style="font-size:.82rem;font-weight:600;color:var(--text-light);text-transform:uppercase;letter-spacing:.3px;display:block;margin-bottom:6px;">Titre *</label>
                            <input type="text" name="titre" class="form-control" placeholder="Résumez votre question en une phrase…" required>
                        </div>
                        <div class="mb-4">
                            <label style="font-size:.82rem;font-weight:600;color:var(--text-light);text-transform:uppercase;letter-spacing:.3px;display:block;margin-bottom:6px;">Message *</label>
                            <textarea name="message" class="form-control" rows="4" placeholder="Développez votre message ici…" required></textarea>
                        </div>
                        <div class="mb-3" style="background:rgba(0,135,255,.06);border:1px solid var(--border);border-radius:10px;padding:12px 16px;">
                            <p style="font-size:.82rem;color:#64748B;margin:0;">🔒 Ta discussion sera publiée sous ton prénom uniquement. Reste bienveillant(e) envers les autres.</p>
                        </div>
                        <button type="submit" class="btn-serenity">
                            <i class="bi bi-send-fill me-2"></i>Publier la discussion
                        </button>
                    </form>
                <?php endif; ?>
            </div>

        </div>
    </div>
</div>

<!-- Bouton retour -->
<div style="text-align:center;margin:40px 0;">
    <a href="index.php" class="btn-hero-outline">
        <i class="bi bi-arrow-left me-2"></i>Retour à l'accueil
    </a>
</div>

<!-- ═══════════ FOOTER ═══════════ -->
<footer class="py-5 mt-4">
    <div class="container">
        <div class="row align-items-center g-3">
            <div class="col-md-4">
                <div class="d-flex align-items-center gap-2 mb-2">
                    <div style="width:32px;height:32px;border-radius:50%;overflow:hidden;border:1px solid var(--border);">
                        <img src="data:image/jpeg;base64,<?= $logoB64 ?>" style="width:100%;height:100%;object-fit:cover;" alt="">
                    </div>
                    <span class="brand">SERENITY</span>
                </div>
                <p style="font-size:.82rem;margin:0;">Là où tout s'apaise.</p>
            </div>
            <div class="col-md-4 text-center">
                <div class="d-flex justify-content-center gap-4 flex-wrap">
                    <a href="index.php">Accueil</a>
                    <a href="ressources.php">Ressources</a>
                    <a href="communaute.php">Communauté</a>
                    <a href="urgences.php">Urgences</a>
                </div>
            </div>
            <div class="col-md-4 text-md-end">
                <p style="font-size:.82rem;margin:0;">© <?= date('Y') ?> SERENITY. Tous droits réservés.</p>
            </div>
        </div>
    </div>
</footer>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
<script>
    function toggleLike(btn) {
        const count = btn.querySelector('.like-count');
        const liked = btn.classList.toggle('liked');
        count.textContent = parseInt(count.textContent) + (liked ? 1 : -1);
    }
</script>
</body>
</html>